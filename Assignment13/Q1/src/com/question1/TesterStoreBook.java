...
//					}
			...
			case 7:
			{
				try(FileOutputStream fout = new FileOutputStream("Books.txt"))
				{
					...
					{
						for (StoreBook s : list) 
						{
							dout.writeUTF(s.getIsbn());
							dout.writeDouble(s.getPrice());
							dout.writeUTF(s.getAuthorName());
							dout.writeByte(s.getQuantity());
			...
			case 8:
			{
				try(FileInputStream fout = new FileInputStream("Books.txt"))
				{
					try(DataInputStream oin = new DataInputStream(fout))
					{
						while(true)
						{
							StoreBook s = new StoreBook();
							s.setIsbn(oin.readUTF());
							s.setPrice(oin.readDouble());
							s.setAuthorName(oin.readUTF());
							s.setQuantity(oin.read());
					...
					catch(Exception e)
				{
					...
					System.out.println("Book is loaded from the file");
			}
			break;
			default:
